...
/**
 * nm_device_vrf_get_hw_address:
 * @device: a #NMDeviceVrf
 *
 * Gets the hardware (MAC) address of the #NMDeviceVrf
 *
 ...
 **/
const char *
nm_device_vrf_get_hw_address (NMDeviceVrf *device)
{
	g_return_val_if_fail (NM_IS_DEVICE_VRF (device), NULL);

	return _nml_coerce_property_str_not_empty (NM_DEVICE_VRF_GET_PRIVATE (device)->hw_address);
}
...
{
	return nm_device_vrf_get_hw_address (NM_DEVICE_VRF (device));
}
              ...
              GParamSpec *pspec)
{
	...
	case PROP_HW_ADDRESS:
		g_value_set_string (value, nm_device_vrf_get_hw_address (device));
		break;
	case PROP_TABLE:
...
finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (nm_device_vrf_parent_class)->finalize (object);
}
...
const NMLDBusMetaIface _nml_dbus_meta_iface_nm_device_vrf = NML_DBUS_META_IFACE_INIT_PROP (
	...
	NML_DBUS_META_IFACE_DBUS_PROPERTIES (
		NML_DBUS_META_PROPERTY_INIT_S      ("HwAddress",  PROP_HW_ADDRESS,   NMDeviceVrf, _priv.hw_address                       ),
...
nm_device_vrf_class_init (NMDeviceVrfClass *klass)
{
	...
	obj_properties[PROP_HW_ADDRESS] =
	    g_param_spec_string (NM_DEVICE_VRF_HW_ADDRESS, "", "",
	                         NULL,
	                         G_PARAM_READABLE |
